...
function createUserActivityToast(user, action) {
    ...
    nameWrapper.textContent = user && user.username ? user.username : 'Пользователь';
...
// ======== Переключатель фоновых частиц (кнопка-таблетка) ========
function setupBackgroundParticlesSwitch() {
  ...
  const applyState = () => {
    btn.setAttribute('aria-pressed', String(bgParticlesEnabled));
    const label = btn.querySelector('.pill-switch-label');
    if (label) label.textContent = bgParticlesEnabled ? 'Частицы: вкл' : 'Частицы: выкл';
    btn.classList.toggle('active', bgParticlesEnabled);
  ...
  btn.addEventListener('click', () => {
    bgParticlesEnabled = !bgParticlesEnabled;
